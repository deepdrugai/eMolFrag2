...
class Brick(Molecule.Molecule):
    def __init__(self, rdkit_mol, parent, suffix=0):
        ...
        self.filename = self.makeFragmentFileName(parent.getFileName(), prefix=constants.BRICK_PREFIX, numeric_suffix=suffix)
    ...
    def toSDF(self):
        ...
        #                2 N.4
        atom_conn_map = {
            index: atom.GetProp(constants.ATOM_CONNECTION_PROP)
            for index, atom in enumerate(self.rdkitObject.GetAtoms())
            if atom.HasProp(constants.ATOM_CONNECTION_PROP)
        }

        appendix = "".join(
            "".join(f"{atom_index + 1} {atomtype}\n" for atomtype in atom_conn_map[atom_index].split(" ")) for atom_index in atom_conn_map
        )
